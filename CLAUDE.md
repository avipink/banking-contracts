...
# ── Banking Contracts Library Context ──

## Role
Shared Kotlin contract library — the explicit API boundary between Core services and BFF.
This is a pure library (no Spring, no runnable app). Published to mavenLocal.

## Ecosystem
- Orchestrator/specs: ../banking-platform/specs/
- Consumed by: accounts-core-svc, payments-core-svc, banking-bff
- This repo defines the CONTRACT — all consumers must use these types at API boundaries

## Package Structure
com.digitalbank.contracts/
├── accounts/   — AccountResponse, AccountSummary, AccountType, AccountStatus, AccountError
├── payments/   — PaymentRequest, PaymentResponse, PaymentStatus, PaymentType, PaymentError
└── common/     — MonetaryAmount, PaginatedResponse, ApiError, AuditMetadata

## Architecture Rules
- NO Spring dependencies — pure Kotlin library
- All types use @Serializable (kotlinx-serialization-json)
- Sealed classes for domain errors (typed, exhaustive)
- Any change to an existing type is a BREAKING CHANGE — requires spec + human review
- Version: group=com.digitalbank, version=1.0.0

## Build & Publish
- ./gradlew build
- ./gradlew publishToMavenLocal
- Consumed via: includeBuild("../banking-contracts") in each Spring Boot service
